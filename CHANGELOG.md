...
> **PR #132** (`fix/sync-emit-hook-from-llm`) closed as stale: the bundled `inst/hooks/llmtelemetry_emit.sh` template was a May-20 snapshot; the live `~/.claude/hooks/` version has since hardened (PPID-anchored session resolution, multi-session-safe global-sentinel gate, absolute `/usr/bin/python3`). Re-sync fresh rather than merge a point-in-time copy. Underlying issue is template drift with no automated sync.
> **Issue #204** (confidential parquet blobs in git history) verified clean: live PIT `sessions.parquet` (HEAD, 560 rows) has 0 `mycare`/`solwatch`/`swarms` rows. The 18 residual rows (15 mycare + 2 solwatch + 1 swarms) survive only at old commit SHAs and are session-metadata only (`session_id, project, started_at, ended_at, duration_min, agent, source`; `working_dir`=NULL) — no PHI content. Remains ACCEPT/track-only; no action.
> **Filed llm#652** (canonical_projects audit: bucket runtime-ephemeral tokens (B) + alias ~12 real projects (A)). The session-start `UNKNOWN=437` is overwhelmingly machine noise (108 `agent-<hash>`, 119 `worktree-<id>`, ~130 branch/session-dir names, 6 test fixtures); only ~12 are real unaliased projects. This is **by-design consumer-filter cardinality** in the llm-side `unified.duckdb sessions.project` column (canonical_projects epic #528 chose consumer-filter + audit, not producer rewrite) — an llm-repo task, NOT llmtelemetry. The llmtelemetry dashboard is already clean (`canonicalize_project()` collapses these at export).
...
>
> **Failed approach:** Phase 5a's fixer agent updated `export_dashboard_data.R`'s own QA section but missed the separate Python QA gate in `deploy-dashboard.yaml`. Lesson: CI YAML QA checks are a separate surface from the R script's own QA validation — both must be updated when file sets change.
