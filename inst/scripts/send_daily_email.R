...
<!-- QA:roborev_section=1 -->',
  ...
  codexbar_html <- tryCatch({
    ...
    if (!has_cb_usage && !has_cb_cost) {
      ...
      # Derive updated_at from usage_tbl (primary source) falling back to cost JSON.
      cb_updated_at <- tryCatch({
        if (!is.null(usage_tbl) && nrow(usage_tbl) > 0 &&
            "updated_at" %in% names(usage_tbl) &&
            !is.na(usage_tbl$updated_at[1L])) {
          as.character(usage_tbl$updated_at[1L])
        } else if (has_cb_cost) {
          raw_cost <- tryCatch(
            jsonlite::fromJSON(cb_cost_path, simplifyVector = FALSE),
            error = function(e) NULL
          )
          if (!is.null(raw_cost) && is.list(raw_cost) && length(raw_cost) > 0L) {
            as.character(raw_cost[[1L]][["updatedAt"]] %||% NA_character_)
          } else NA_character_
        } else NA_character_
