# export_dashboard_data.R CI fallback copies ccusage_daily.json (not ccusage_daily_all.json)
    ...
    Output
       ...
       [9] "  }"                                                                                          
      [10] "  # Always overwrite sessions/blocks so the fallback is deterministic (no stale"              
      [11] "  # snapshot preserved from a prior run or checked-in file). Fixes #141."                     
      [12] "  write_json(list(), file.path(out_dir, \"ccusage_sessions.json\"), auto_unbox = TRUE)"       
      [13] "  cat(\"  -> wrote empty ccusage_sessions.json (CI fallback; cmonitor-rs not available)\\n\")"
      [14] "  write_json(list(), file.path(out_dir, \"ccusage_blocks.json\"), auto_unbox = TRUE)"         
      [15] "  cat(\"  -> wrote empty ccusage_blocks.json (CI fallback; cmonitor-rs not available)\\n\")"  
      [16] "}"                                                                                            
