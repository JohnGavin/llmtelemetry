...
ui <- page_navbar(
  ...
  header = tagList(
    tags$style(HTML("
      ...
      /* DT tables: auto-height based on rows, clickable cursor */
      .dataTables_scrollBody { max-height: none !important; }
      .dataTables_wrapper { cursor: zoom-in; }
      /* Fullscreen table styling */
      #fullscreen-overlay .dataTables_wrapper { font-size: 16px; }
      #fullscreen-overlay table.dataTable { color: #e0e0e0 !important; }
      #fullscreen-overlay table.dataTable thead th { color: #e0e0e0 !important; background-color: #2a2a2a !important; }
      #fullscreen-overlay table.dataTable tbody td { border-color: #444 !important; }
      /* Fullscreen overlay */
      #fullscreen-overlay {
    ...
    tags$script(HTML("
      ...
      function openOverlay(el) {
        ...
        // Re-render plotly at full size if applicable
        if (window.Plotly && clone.data) {
          window.Plotly.relayout(clone, { width: window.innerWidth - 60, height: window.innerHeight - 60 });
        }
      }

      // Click on plotly chart OR DT table -> fullscreen
      document.addEventListener('click', function(e) {
        if (ov.style.display === 'flex') return;
        // Check for plotly
        var plot = e.target.closest('.js-plotly-plot');
        if (plot) { e.stopPropagation(); openOverlay(plot); return; }
        // Check for DT table
        var dt = e.target.closest('.dataTables_wrapper');
        if (dt) { e.stopPropagation(); openOverlay(dt); return; }
      }, true);
