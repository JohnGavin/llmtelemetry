...
dark_axis <- list(
  ...
  tickfont = list(color = "#e0e0e0", size = 13),
  titlefont = list(color = "#ffffff", size = 14)
)
dark_layout <- function(p, title = NULL, xaxis = list(), yaxis = list(), ...) {
  p |> layout(
    ...
    legend = list(
      ...
      bgcolor = "rgba(50,50,50,0.95)", bordercolor = "#888", borderwidth = 1
    ),
    hoverlabel = list(bgcolor = "white", font = list(color = "black", size = 13)),
    margin = list(b = 90, t = 40),
    ...
...
ui <- page_navbar(
  ...
  header = tagList(
    tags$style(HTML("
      ...
      .js-plotly-plot { cursor: zoom-in; }
      .card .plotly { min-height: 500px; }
      ...
      #fullscreen-overlay .plotly, #fullscreen-overlay .js-plotly-plot {
        width: 100% !important; height: calc(100vh - 20px) !important;
      }
      /* Hint banner */
      #fullscreen-hint {
        position: absolute; top: 5px; right: 20px; color: #999;
    ...
    tags$script(HTML("
      ...
      // Click on any plotly chart -> clone into fullscreen overlay
      document.addEventListener('click', function(e) {
        var plot = e.target.closest('.js-plotly-plot');
        if (!plot) return;
        if (ov.style.display === 'flex') return; // already open
        e.stopPropagation();
        var clone = plot.cloneNode(true);
        // Remove old plot content (keep only the clone)
        while (ov.childNodes.length > 1) ov.removeChild(ov.lastChild);
        ov.appendChild(clone);
        ov.style.display = 'flex';
        ov.style.alignItems = 'center';
        ov.style.justifyContent = 'center';
        // Re-render plotly in clone at full viewport size
        if (window.Plotly && clone.data) {
          window.Plotly.relayout(clone, { width: window.innerWidth - 40, height: window.innerHeight - 40 });
        }
      }, true);
  ...
  nav_panel("Overview", value = "Overview",
    ...
    layout_columns(
      col_widths = c(12),
      card(card_header("Daily Cost (All Sources)"), plotlyOutput("overview_cost_ts", height = "600px"),
  ...
  nav_panel("Cost Trends", value = "Cost Trends",
    layout_columns(
      col_widths = c(6, 6),
      card(card_header("Daily Cost by Project"), plotlyOutput("cost_by_project", height = "550px"),
        card_footer(class = "text-muted small", "Daily Claude cost by project (lines+markers)")),
      card(card_header("Cumulative Cost"), plotlyOutput("cost_cumulative", height = "550px"),
        card_footer(class = "text-muted small", "Running total of cost over time by source"))
    ),
    layout_columns(
      col_widths = c(6, 6),
      card(card_header("Weekly Cost by Source"), plotlyOutput("cost_weekly", height = "550px"),
        card_footer(class = "text-muted small", "Weekly cost grouped by Claude vs Gemini")),
      card(card_header("Weekly Cost by Model"), plotlyOutput("cost_by_model", height = "550px"),
  ...
  nav_panel("Token Analysis", value = "Token Analysis",
    layout_columns(
      col_widths = c(12),
      card(card_header("Daily Tokens"), plotlyOutput("token_daily", height = "600px"),
        card_footer(class = "text-muted small", "Daily token consumption across all sources"))
    ),
    layout_columns(
      col_widths = c(6, 6),
      card(card_header("Token Composition"), plotlyOutput("token_comp", height = "550px"),
        card_footer(class = "text-muted small", "Grouped bars: input, output, cache-create, and cache-read tokens")),
      card(card_header("Weekly Tokens by Source"), plotlyOutput("token_weekly", height = "550px"),
  ...
  nav_panel("Sessions", value = "Sessions",
    ...
    layout_columns(
      col_widths = c(6, 6),
      card(card_header("Session Cost Distribution"), plotlyOutput("sess_hist", height = "550px"),
        card_footer(class = "text-muted small", "Histogram of per-session costs")),
      card(card_header("Sessions Over Time"), plotlyOutput("sess_scatter", height = "550px"),
  ...
  nav_panel("Time Blocks", value = "Time Blocks",
    ...
    layout_columns(
      col_widths = c(6, 6),
      card(card_header("Block Timeline"), plotlyOutput("block_timeline", height = "550px"),
        card_footer(class = "text-muted small", "Gantt-style block start/end times colored by cost")),
      card(card_header("Hourly Activity Heatmap"), plotlyOutput("block_heatmap", height = "550px"),
  ...
  nav_panel("Commits", value = "Commits",
    ...
    layout_columns(
      col_widths = c(12),
      card(card_header("Lines Changed per Commit"), plotlyOutput("commit_hist", height = "600px"),
        card_footer(class = "text-muted small", "Distribution of total lines changed per commit"))
    ),
    layout_columns(
      col_widths = c(6, 6),
      card(card_header("Lines Changed Over Time"), plotlyOutput("commit_scatter_time", height = "550px"),
        card_footer(class = "text-muted small", "Lines changed per commit over time, sized by files changed")),
      card(card_header("Additions vs Deletions"), plotlyOutput("commit_scatter_add_del", height = "550px"),
...
server <- function(input, output, session) {
  ...
  # Fullscreen toggle — no-op, handled by injected JS below
