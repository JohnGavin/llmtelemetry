...
ui <- page_navbar(
  ...
  nav_panel("Commits", value = "Commits",
    layout_columns(
      ...
      value_box("P90 Lines", textOutput("vb_p90_lines"), theme = "warning")
    ),
    layout_columns(
      col_widths = c(6, 6),
      card(card_header("Lines Changed Distribution"), plotlyOutput("commit_hist", height = "400px"),
        card_footer(class = "text-muted small", "Histogram of total lines changed (added + deleted) per commit")),
      card(card_header("Lines Changed Over Time"), plotlyOutput("commit_ts", height = "400px"),
        card_footer(class = "text-muted small", "Daily total lines changed with 7-day rolling average"))
    ),
    layout_columns(
      col_widths = c(12),
      card(card_header("Recent Commits"), DTOutput("commit_table"),
        card_footer(class = "text-muted small", "Git commit history with lines added, deleted, and files changed"))
    )
  ),
...
server <- function(input, output, session) {
  ...
  output$vb_commits <- renderText({
    as.character(nrow(git_d()))
  })
  output$vb_median_lines <- renderText({
    d <- git_d()
    if (nrow(d) == 0) return("0")
    as.character(median(d$lines_changed))
  })
  output$vb_mean_lines <- renderText({
    d <- git_d()
    if (nrow(d) == 0) return("0")
    as.character(round(mean(d$lines_changed)))
  })
  output$vb_p90_lines <- renderText({
    d <- git_d()
    if (nrow(d) == 0) return("0")
    as.character(round(quantile(d$lines_changed, 0.9)))
  })
  ...
  output$commit_hist <- renderPlotly({
    ...
    plot_ly(d, x = ~lines_changed, type = "histogram", nbinsx = 30,
      marker = list(color = "#2c3e50")) |>
      layout(
        xaxis = list(title = "Lines Changed per Commit"),
        yaxis = list(title = "Count")
      ) |> config(scrollZoom = TRUE)
  })

  output$commit_ts <- renderPlotly({
    d <- git_d()
    if (nrow(d) == 0) return(plotly_empty())
    daily <- d |>
      group_by(date) |>
      summarise(lines = sum(lines_changed), commits = n(), .groups = "drop") |>
      arrange(date) |>
      mutate(rolling_avg = {
        n <- length(lines)
        ra <- rep(NA_real_, n)
        for (i in 7:n) ra[i] <- mean(lines[(i - 6):i])
        ra
      })
    plot_ly(daily, x = ~date, y = ~lines, type = "bar", name = "Daily Lines",
      marker = list(color = "#bdc3c7")) |>
      add_trace(x = ~date, y = ~rolling_avg, type = "scatter", mode = "lines",
        name = "7-day Avg", line = list(color = "#e74c3c", width = 2)) |>
      layout(
        xaxis = list(title = ""),
  ...
  output$commit_table <- renderDT({
    d <- git_d()
    if (nrow(d) == 0) return(datatable(tibble()))
    d |>
      select(Hash = hash, Date = date, Added = lines_added,
             Deleted = lines_deleted, Changed = lines_changed,
             Files = files_changed, Message = message) |>
      datatable(options = list(pageLength = 15, dom = "frtip", scrollX = TRUE),
        rownames = FALSE)
  })
}
